#!/bin/bash
WORK_PATH='D:\work\code\08nginxdockercicd\vue-back' # 工作目录（服务器端）
cd $WORK_PATH
echo "先清除旧的代码"
...
docker build -t vue-back:1.0 . # .表示当前目录(上面定义的WORK_PATH)
# 去当前目录的vue-back下找Dockerfile文件进行构建
# 会根据这个dockerfile文件去生成镜像，-t：生成的镜像名字叫vue-back:1.0
# 不加1.0每次就都是laster
